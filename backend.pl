...
helper split_hierarchical_group_async => sub {
    ...
    for (my $i = 0; $i < @$characteristics; $i++) {
        ...
        if (my $sym = $char->{symptom}) {
            ...
            my $exclude = $sym->{exclude} // \0;
            my $sym_comb = $sym->{combinationMethod} // "all-of";
            my $index = $i;

            my @label_promises;
            foreach my $lbl (@$labels) {
                push @label_promises, $self->split_symptom_components_async($lbl, $client_model);
            }

            my $p = Mojo::Promise->all(@label_promises)->then(sub {
                my @splits = @_;
                my @processed_nodes;
                ...
                for (my $j = 0; $j < @splits; $j++) {
                    ...
                    if ($res && ref $res eq 'HASH') {
                        my $phenotype = $res->{phenotype};
                        my $modifiers = $res->{modifiers} // [];
                ...
                if (@processed_nodes == 1) {
                    $characteristics->[$index] = $processed_nodes[0];
                } elsif (@processed_nodes > 1) {
                    $characteristics->[$index] = {
                        subgroup => {
                            combinationMethod => $sym_comb,
                            exclude => $exclude,
                            characteristics => \@processed_nodes
                        }
                    };
                }
            });
            ...
            for (my $j = 0; $j < @$labels; $j++) {
                ...
                my $p_lbl = $self->map_to_hpo_async($lbl, $is_modifier)->then(sub {
                    ...
                    $codings[$lbl_idx] = {
                        ...
                        is_modifier => $is_modifier ? \1 : \0 # blau oder grau?
                    };
                });
...
post '/DBB/extract_phenopacket' => sub {
    ...
    my $sys_instruction = "You are an expert medical entity extraction assistant. Your job is to extract patient phenotype abnormalities, severities, onsets, and individual phenotypic modifiers strictly conforming to the requested schema. Do not make up categories or invent details not stated.";
    my $user_prompt     = "Analyze the patient clinical description and extract the exact phenotypic structures.";

    $c->extract_structured_data_async($text_content, $phenotypic_features_schema, $sys_instruction, $user_prompt)->then(sub {
        my $extracted_data = shift;
        my $raw_features = $extracted_data->{phenotypicFeatures} // [];

        # Conduct dense mapping tasks asynchronously
        my @feature_promises;
        ...
        if (!@feature_promises) {
            return $c->render(json => { error => "No phenotypic features identified during pipeline execution." }, status => 400);
        }
...
app->config(hypnotoad => {listen => ['http://*:4006'], workers => 3, heartbeat_timeout=>120, inactivity_timeout=> 120});
app->start;
